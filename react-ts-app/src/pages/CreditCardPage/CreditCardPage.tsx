...
import { fetchCreditCard } from '../../store/slices/creditCardSlice';

// Dispatches its Redux slice's fetch thunk (backed by
// src/services/creditCardService.ts) on mount, falling back to this page's
...
export function CreditCardPage() {
  ...
  const { data, status } = useAppSelector((state) => state.creditCard);

  useEffect(() => {
    if (status === 'idle') dispatch(fetchCreditCard());
  }, [status, dispatch]);

  const title = data?.title ?? 'Black credit card';
  const description = data?.description ?? 'Get the Credit Card that suits your lifestyle';
