...
// ── Stamp constructors ────────────────────────────────────────────────────────
function PersonalStamp(title, fullname, surname, idnumber, mobile) {
  this.title = { value: title.value, label: title.label };
...
function IncomeStamp(grosssalary, income, frequency, expenses) {
  this.grossSalary = grosssalary;
  this.netIncome = income;
  this.frequency = { value: frequency.value, label: frequency.label };
  this.expenses = expenses;
}

function EmploymentStamp(
  bank,
  wageType,
  employmentType,
  employerName,
  ...
  this.bank = { value: bank.value, label: bank.label };
  this.reference = "UNKNOWN";
  this.wageType = wageType;
  this.salaryDepositDay = "";
  this.employmentStartDate = employmentStartDate.replace(/-/g, "").trim();
  ...
  this.switchBoardAreacode = "";
}

function OtpStamp(otpValue, uniqueTransactionID, mobile) {
  this.otpValue = otpValue;
  this.uniqueTransactionID = uniqueTransactionID;
  this.mobile = mobile;
}

window.personalStamp = null;
window.incomeStamp = null;
window.employmentStamp = null;
window.otpStamp = null;

function tryStampPersonal() {
  ...
  if (!titleVal || !fullname || !surname || !idnumber || !mobile) {
    window.personalStamp = null;
    return;
  }
  window.personalStamp = new PersonalStamp(
    { value: titleVal, label: $title.find("option:selected").text() },
    fullname,
    surname,
    idnumber,
    mobile,
...
function tryStampIncome() {
  var grosssalary = $.trim($grosssalary.val());
  var income = $.trim($income.val());
  var freqVal = $.trim($grosssalaryWage.val());
  var expenses = $.trim($expenses.val());

  if (!grosssalary || !income || !freqVal || !expenses) {
    window.incomeStamp = null;
    return;
  }
  window.incomeStamp = new IncomeStamp(
    grosssalary,
    income,
    { value: freqVal, label: $grosssalaryWage.find("option:selected").text() },
    expenses,
  );
  console.log("Income stamp:", window.incomeStamp);
}
function tryStampOffers() {
    if (!originalOffers) {
        window.offersStamp = null;
        return;
    }
    window.offersStamp = { offers: originalOffers };
    console.log('Offers stamp:', window.offersStamp);
}
function tryStampEmployment() {
  var bankVal = $.trim($yourbank.val());
  var wageType = $.trim($grosssalaryWage.val());
  var employmentType = $.trim($employeeType.val());
  var employer = $.trim($youremployer.val());
  var occupationStatus = $.trim($occupationstatus.val());
  var startDate = $.trim($startdate.val());
  var occupationType = $.trim($occupationtype.val());
  ...
  window.employmentStamp = new EmploymentStamp(
    { value: bankVal, label: $yourbank.find("option:selected").text() },
    wageType,
    employmentType,
    employer,
    occupationStatus,
  ...
  console.log("Employment stamp:", window.employmentStamp);
}
function tryStampOtp() {
  var otpValue = $.trim($("#oneTimePIN").val());
  var mobile = $.trim($mobile.val());

  if (!otpValue || otpValue.length !== 6 || !uniqueTransactionID) {
    window.otpStamp = null;
    return;
  }
  window.otpStamp = new OtpStamp(otpValue, uniqueTransactionID, mobile);
  console.log("OTP stamp:", window.otpStamp);
}
// ─────────────────────────────────────────────────────────────────────────────
...
async function searchClientById(idNumber) {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: "https://cxpweb.africanbank.co.za/businessBanking/client-search",
      type: "POST",
      headers: {
        "x-api-key": "BYCVJWgQlvAQSc6yZsA2SRunzaERsM85yatqQXE6lL7e7SoOdkTGRIYpO5JCTTZec6zvAenWO0Y0A2SLEgTVeDxXWOZlJwYI6oAhHdDB9HC1q6xW8PRbqurRkzYO6ATu=",
...
$("#idnumber").on("keyup", async function () {
  ...
  if (idNumber.length === 13) {
    try {
      ...
      let found = apps.find(
        (app) =>
          app.applicationType === "CRE" && app.applicationStatus === "NEW",
      );
      ...
      if (found) {
        ...
        document.querySelector("#thankyoumodal .fullname").textContent = name
          ? ", " + name
          : "";
...
">
  ...
  if (
    parsedData?.results?.reasonCodes &&
    parsedData?.results?.reasonCodes.length > 0
  ) {
  ...
  const results = filteredOffers?.results?.offerResponse?.offers
    ? filteredOffers?.results?.offerResponse?.offers
    : filteredOffers;
  const filterResult = originalOffers?.results?.offerResponse?.offers
    ? originalOffers?.results?.offerResponse?.offers
    : results || [];
  ...
  if (filterResult === null) {
    $("#text-error").text(
      originalOffers?.results?.reasonCodes[0]?.reasonCodeDescription,
    );
  ...
  if (filterResult && filter === undefined && filterResult !== null) {
    ...
    uniqueClassifications = Array.from(
      new Map(
        classifications.map((item) => [
          `${item.productClassification}-${item.productDescription}`,
          item,
        ]),
      ).values(),
    );
    ...
    $("#filterData").append(
      '<div class="filter-card active" onclick="filterOffers(\'all\', this)">' +
        "ALL" +
        "</div>",
    );
  }
  ...
  uniqueClassifications?.forEach(function (classification, index) {
    ...
    $("#filterData").append(
      '<div class="filter-card" onclick="filterOffers(\'' +
    ...
    success: function (data) {
      ...
      if (results.serviceHeaderResponse.resultCode === 200) {
        ...
        results?.universalBankBranches?.forEach((element) => {
          $("#yourbank").append(
            `<option value=${element.bankCode}%${element?.branchCode}>${element.bankName}</option>`,
          );
        });
        ...
        $(".bank-not-found").val(
          results.serviceHeaderResponse.resultDescription,
        );
...
$("#youremployer").autocomplete({
  source: function (request, response) {
    $.ajax({
      ...
      contentType: "application/json; charset=utf-8",
      url:
        "/umbraco/surface/QuickLoans/SearchEmployer?employerName=" +
        $("#youremployer").val() +
        "&employeeType=" +
        $("#employeeType").val(),
      dataType: "json",
  ...
  select: function (event, ui) {
    ...
    tryStampEmployment();
    if (isEmployeeStepComplete()) postEmployeeDetails();
  },
...
function filterOffers(type, navDiv) {
  ...
  const filteredOffers =
    type === "all"
      ? originalOffers
      : copyOffer.filter(
          (offer) => offer?.offerDetails?.productClassification === type,
        );
...
if (urlParams) {
  ...
  if (affiliateData.grossSalaryType) {
    document.getElementById("grosssalary-select").value =
      affiliateData.grossSalaryType;
  }
  ...
  if (affiliateData.occupationStatus) {
    document.getElementById("occupationstatus").value =
      affiliateData.occupationStatus;
  }
  if (affiliateData.occupationType) {
    document.getElementById("occupationtype").value =
      affiliateData.occupationType;
  }
  if (affiliateData.datetimeInput) {
    document.getElementById("datetimeInput").value =
      affiliateData.datetimeInput;
  }
...
$.ajax({
  url: `https://cxpweb.africanbank.co.za/affiliate/api/data/${submissionId}`,
  method: "GET",
  dataType: "json",
  success: function (response) {
    ...
    if (affiliateData.Amount) {
      const numericAmount = parseInt(
        affiliateData.Amount.replace(/\D/g, ""),
        10,
      );
      if (!isNaN(numericAmount)) {
        document.getElementById("input-Amount1").value = numericAmount;
    ...
    if (affiliateData.EmployeeType) {
      document.getElementById("employeeType").value =
        affiliateData.EmployeeType;
    }

    if (affiliateData.YourEmployer) {
      document.getElementById("youremployer").value =
        affiliateData.YourEmployer;
    }
...
function toggleIcons() {
  ...
  const icons = document.querySelectorAll(
    ".header_tab.completed .material-icons",
  );
  icons.forEach((icon) => {
...
$(document).ready(function () {
  ...
  function validateSAID(id) {
    ...
    if (now.getMonth() < mm || (now.getMonth() === mm && now.getDate() < dd)) {
      age--;
    }
  ...
  $("#fullname").on("input", function () {
    ...
    if (fullname && !$error.is(":visible")) {
      $("#loanOfferTitle").text(
        "Hi " +
          fullname +
          ', Great News! You Qualify - view your <span class="dynamic-loan-amount">personalised loan quick quotes below.</span>',
      );
    } else {
  ...
  $("#youremployer").on("blur input", function () {
    ...
    if (
      !inputVal ||
      !validEmployerSelected ||
      inputVal !== selectedEmployerName
    ) {
      $input.addClass("invalid");
      $error
        .text("Please select a valid employer from the list.")
        .css("display", "block");
    } else {
      $input.removeClass("invalid");
      $error.hide();
  ...
  $("#datetimeInput").on("input change", function () {
    ...
    if (selectedDate > fiveMonthsAgo) {
      $input.addClass("invalid");
      $error
        .text("Eligibility begins after 5 months of employment")
        .css("display", "block");
    } else {
      $input.removeClass("invalid");
      $error.hide();
...
function isPersonalStepComplete() {
  return (
    $("#idnumber").val().trim() !== "" &&
    !$("#idnumber").hasClass("invalid") &&
    $("#fullname").val().trim() !== "" &&
    !$("#fullname").hasClass("invalid") &&
    $("#surname").val().trim() !== "" &&
    !$("#surname").hasClass("invalid") &&
    $("#mobile").val().trim().length === 10 &&
    !$("#mobile").hasClass("invalid")
  );
}

function isFinancialStepComplete() {
  return (
...
function isEmployeeStepComplete() {
  return (
    $("#yourbank").val().trim() !== "" &&
...
// ── Blur-triggered DB stamping ────────────────────────────────────────────────
// Personal step → only stamp when ALL personal fields are valid
$("#idnumber, #fullname, #surname").on("blur", function () {
  if (isPersonalStepComplete()) postPersonalDetails();
});

// Mobile: lookup existing record first, pre-populate, block if already submitted,
// then stamp personal step only when all personal fields are complete
$("#mobile").on("blur", function () {
  ...
  $.ajax({
    url:
      "/umbraco/surface/QuickLoans/GetQQByPhone?phoneNumber=" +
      encodeURIComponent(mobile),
    type: "GET",
    dataType: "json",
    success: function (response) {
      const entry = response && response.data;
      ...
      // Block user if they already accepted an offer
      if (entry.SelectedOffer) {
        $("#text-error").text(
          "You already have an application in progress. Please contact us for assistance.",
        );
        openDialog("thankyoumodal");
        return;
      }
      ...
      if (entry.PersonalDetailsRequestString) {
        try {
          const personal = JSON.parse(entry.PersonalDetailsRequestString);
          if (personal.idNumber) {
            $("#idnumber").val(personal.idNumber).trigger("input");
            const gd = parseInt(personal.idNumber.slice(6, 10), 10);
            if (!isNaN(gd)) $("#title").val(gd >= 5000 ? "Mr" : "Ms");
          }
          if (personal.firstName) {
            $("#fullname")
              .val(personal.firstName.split(";")[0].trim())
              .trigger("input");
          }
          if (personal.surname)
            $("#surname").val(personal.surname).trigger("input");
          if (personal.EmailAddress) $("#email-id").val(personal.EmailAddress);
        } catch (e) {
      ...
      if (entry.FinancialDetails) {
        try {
          const fin = JSON.parse(entry.FinancialDetails);
          const incomeList = fin.applicationIncomeList || [];
          const nett = incomeList.find(function (i) {
            return i.incomeType === "NETT";
          });
          const gross = incomeList.find(function (i) {
            return i.incomeType === "GROSS";
          });
          if (nett && nett.incomeValue)
            $("#income").val(nett.incomeValue).trigger("input");
          if (gross && gross.incomeValue)
            $("#grosssalary").val(gross.incomeValue).trigger("input");
        } catch (e) {
          console.warn("Could not parse financial details:", e);
        }
      }
      ...
      if (entry.EmployeeDetails) {
        try {
          const emp = JSON.parse(entry.EmployeeDetails);
          if (emp.wageType)
            $("#frequencytype").val(emp.wageType).trigger("change");
          if (emp.employmentType)
            $("#employeeType").val(emp.employmentType).trigger("change");
          if (emp.employerName) {
            $("#youremployer").val(emp.employerName);
            validEmployerSelected = true;
            selectedEmployerName = emp.employerName;
          }
          if (emp.occupationStatus)
            $("#occupationstatus").val(emp.occupationStatus).trigger("input");
          if (emp.occupationType)
            $("#occupationtype").val(emp.occupationType).trigger("input");
          if (emp.employmentStartDate) {
            const d = emp.employmentStartDate.replace(/-/g, "");
            const formatted =
              d.length === 8
                ? d.slice(0, 4) + "-" + d.slice(4, 6) + "-" + d.slice(6, 8)
                : emp.employmentStartDate;
            $("#datetimeInput").val(formatted).trigger("change");
            if (fp) fp.setDate(formatted, false);
          }
        } catch (e) {
          console.warn("Could not parse employee details:", e);
        }
      }

      // Stamp each step that is now complete after population
      if (isPersonalStepComplete()) postPersonalDetails();
      if (isFinancialStepComplete()) postFinancialDetails();
      if (isEmployeeStepComplete()) postEmployeeDetails();
      checkAllValid();
    },
    error: function () {
...
// Financial step → stamp then post when ALL financial fields are valid
$("#grosssalary, #income, #expenses").on("blur", function () {
  tryStampIncome();
  if (isFinancialStepComplete()) postFinancialDetails();
});
$("#frequencytype").on("change", function () {
...
// Employee step → stamp then post when ALL employee fields are valid
$("#youremployer, #occupationstatus, #occupationtype, #datetimeInput").on(
  "blur",
  function () {
    tryStampEmployment();
    if (isEmployeeStepComplete()) postEmployeeDetails();
...
$("#yourbank, #employeeType").on("change", function () {
  tryStampEmployment();
  if (isEmployeeStepComplete()) postEmployeeDetails();
});
// ─────────────────────────────────────────────────────────────────────────────

// ── Stamps update on both focus and blur/change so the last field filled is captured ──
$title
  .add($fullname)
  .add($surname)
  .add($idnumber)
...
*/
  ...
  if (
    !(
      isSouthAfricanIDValid(idnumber.value) ||
      isPassportNumberValid(idnumber.value)
    )
  ) {
...
function validateIncomeDetails() {
  ...
  if (!grosssalary.value.trim() || grossValue <= 0) {
    ...
    showError(
      grosssalaryError,
      ` The Gross salary must be greater ${incomeValues}.`,
    );
    isValid = false;
  } else {
  ...
  if (!income.value.trim() || incomeValue <= 0) {
    ...
    showError(
      incomeError,
      ` The income salary must be less than ${grossValues}.`,
    );
    isValid = false;
  } else {
...
function validateEmploymentDetails() {
  ...
  if (
    ...
    showError(
      youremployerError,
      "Please select a valid employer from the list.",
    );
    isValid = false;
  } else {
  ...
  const occupationstatusError = document.getElementById(
    "occupationstatus-error",
  );
  ...
  if (!startdate.value.trim()) {
    ...
    if (selectedDate > fiveMonthsAgo) {
      ...
      showError(
        startdateError,
        "Eligibility begins after 5 months of employment",
      );
      isValid = false;
    } else {
      startdate.classList.remove("invalid");
...
function toggleAccordion(id) {
  ...
  var personObjs = {
    ...
    firstName: tokenAffiliate
      ? $fullname.val() + ";Affiliate-Token" + tokenAffiliate
      : $fullname.val(),
  ...
  icon.innerText =
    content.style.display === "block" ? "expand_less" : "expand_more";

  // Auto-focus first invalid or empty input when incomeExpenses accordion opens
  if (!isOpen && id === "incomeExpenses") {
    setTimeout(() => {
      const firstTarget =
        content.querySelector("input.invalid, select.invalid") ||
        content.querySelector('input:not([type="hidden"]):not([disabled])');
...
function submitLoanAmount(modalId) {
  ...
  if (loanAmount < min || loanAmount > max || isNaN(loanAmount)) {
    ...
    let formattedLoanAmount =
      "R" + Number(loanAmount).toLocaleString("en-ZA").replace(/,/g, " ");
...
function openExpensesModal(modalId) {
  openDialog(modalId);
}

function closeExpensesModal(modalId, event) {
  if (isFinancialStepComplete()) postFinancialDetails();
  closeDialog(modalId, event);
}

function verifyexpensive(modalId) {
  const rent =
    parseInt(document.getElementById("rent").value.replace(/[^\d]/g, "")) || 0;
  const transport =
    parseInt(
      document.getElementById("transport").value.replace(/[^\d]/g, ""),
    ) || 0;
  const groceries =
    parseInt(
      document.getElementById("groceries").value.replace(/[^\d]/g, ""),
    ) || 0;
  const others =
    parseInt(document.getElementById("others").value.replace(/[^\d]/g, "")) ||
    0;
...
function verifyOTP(modalId) {
  ...
  function showError(msg) {
    ...
    $("#errors")
      .html(`<p class="notification">${sanitize(msg)}</p>`)
      .show();
  }
  ...
  // AJAX Call
  // -------------------------------
  fetch("/umbraco/surface/QuickLoans/GetOffers", {
    method: "POST",
    headers: {
      "Content-Type": "application/json; charset=utf-8",
      __RequestVerificationToken: token,
    },
    body: JSON.stringify({ form: post_data }),
  })
    .then(async (response) => {
      ...
      if (
        typeof raw === "string" &&
        (raw.includes("(500)") || raw.includes("(504)"))
      ) {
      ...
      const exists = reasonCodes.some(
        ({ reasonCode, reasonCodeDescription }) =>
          reasonCode != null && reasonCodeDescription != null,
      );
      ...
      if (exists) {
        $("#otpModal").modal("hide");
        // If not Bootstrap, use: $('#otpModal').hide();

        offersContainer.innerHTML = `
        <div class="unsuccessful-container">
            <img src="/media/n20dlrc0/process-completed-icon-1.svg"
                 alt="Unsuccessful icon"
                 class="unsuccessful-icon">

            <h2 class="color-brand-1 mb-20 text-center">Unsuccessful!</h2>

            <p class="font-md color-brand-1 mb-20 text-center">
                At this stage, you don’t meet African Bank’s credit policy requirements.<br>
                Don’t worry — you can always try again in future when your circumstances improve.
            </p>
    ...
    `;
      ...
      if (resultCode === 200) {
        ...
        document.getElementById("personalDetailsAccordion").style.display =
          "none";

        const personalAccordion = document.querySelector(
          ".accordion[onclick*='personalDetailsAccordion']",
        );
        ...
        if (personalAccordion) {
          ...
          if (!checkIcon) {
            ...
            personalAccordion.insertBefore(
              checkIcon,
              personalAccordion.firstChild,
            );
          } else {
...
function updateSliderText(checkbox) {
  ...
  updateFormData = {
    debtReview: debtReview,
    insolvent: insolvent,
    popiaConsent: popiaConsent,
    popiaPermission: popiaPermission,
  };

  const confirmBtn = document.querySelector(
    "#creditQuestionsModal .modal_footer .btn-primary[onclick*='submitCreditQuestions']",
  );
  const selectAllBtn = document.querySelector(
    "#creditQuestionsModal .modal_footer .btn-link",
  );
...
function submitCreditQuestions(modalId) {
  ...
  const popiaPermissionGiven =
    document.getElementById("popiaPermission").checked;
...
function otpStepper(person) {
  $.ajax({
    url: "/umbraco/surface/QuickLoans/LogOTPOfferDetail",
    type: "POST",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({ qQStepLog: person }),
    success: function (data) {
      $("#form-preloader").hide();
...
function postPersonalDetails() {
  ...
  const token = $('input[name="__RequestVerificationToken"]')[0]?.value;
  const personData = {
    idNumber: $idnumber.val(),
    title: $title.val(),
    surname: $surname.val(),
  ...
  const post_data = {
    PersonalDetailsRequestString: JSON.stringify(personData),
    PhoneNumber: mobile,
  ...
  $.ajax({
    ...
    success: function () {
      LogMarketingConsentDetails(JSON.stringify(updateFormData));
      $("#form-preloader").hide();
      SaveMarketingConsentDetails(personData);
    },
    error: function () {
      $(".pause-form").hide();
    },
...
function postFinancialDetails() {
  ...
  const others =
    parseInt(
      (document.getElementById("others")?.value || "").replace(/[^\d]/g, ""),
    ) || 0;
  ...
  const finances = {
    applicationIncomeList: [
      {
        incomeType: "NETT",
        incomeValue: $income.val()?.replace(",", "")?.trim(),
      },
      {
        incomeType: "GROSS",
        incomeValue: $grosssalary.val()?.replace(",", "")?.trim(),
      },
    ],
    applicationExpensesList: [
      { expenseType: "TRANSPRT", expenseValue: transport },
      { expenseType: "FOOD", expenseValue: groceries },
      { expenseType: "RENT", expenseValue: rent },
      { expenseType: "OTHER", expenseValue: others },
    ],
  };
  const post_data = {
    PersonalDetailsRequestString: JSON.stringify({}),
    PhoneNumber: mobile,
    FinancialDetails: JSON.stringify(finances),
    OTPValue: null,
    Offers: null,
    SelectedOffer: null,
    utm_source: $("#utmSource").val(),
  ...
  $.ajax({
    headers: { __RequestVerificationToken: token },
    url: "/umbraco/surface/QuickLoans/LogFinancialDetails",
    type: "POST",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({ qQStepLog: post_data }),
    success: function () {},
    error: function () {},
  });
}

function LogMarketingConsentDetails(MarketingConsent) {
  const mobile = $mobile.val().trim();
  if (mobile.length < 10) return;
  ...
  const post_data = {
    PersonalDetailsRequestString: null,
    PhoneNumber: mobile,
  ...
  $.ajax({
    headers: { __RequestVerificationToken: token },
    url: "/umbraco/surface/QuickLoans/LogMarketingConsentDetails",
    type: "POST",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({ qQStepLog: post_data }),
...
function postEmployeeDetails() {
  ...
  const employments = {
    reference: "UNKNOWN",
    wageType: $grosssalaryWage.val(),
    salaryDepositDay: "",
    employmentStartDate: $startdate.val().replace(/-/g, "").trim(),
    occupationType: $occupationtype.val(),
    employmentType: $employeeType.val(),
    occupationStatus: $occupationstatus.val(),
    contractEndDate: "",
    employerName: $youremployer.val(),
    calenderId: "",
    employeeNumber: "",
    switchBoardNumber: "",
    switchBoardAreacode: "",
  ...
  const post_data = {
    ...
    PhoneNumber: mobile,
    FinancialDetails: null,
    EmployeeDetails: JSON.stringify(employments),
    OTPValue: null,
    Offers: null,
    SelectedOffer: null,
    utm_source: $("#utmSource").val(),
  ...
  $.ajax({
    headers: { __RequestVerificationToken: token },
    url: "/umbraco/surface/QuickLoans/LogEmployeeDetails",
    type: "POST",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({ qQStepLog: post_data }),
...
function openCreditCheckModal() {
  ...
  const isValidacccordians =
    isPersonalValid && isIncomeValid && isEmploymentValid;

  if (isValidacccordians) {
    document
      .getElementById("creditBureauCheckModal")
  ...
  var checkIncomeDetails = document
    .getElementById("income_accordion")
  ...
  var checkEmploymentDetails = document
    .getElementById("employment_accordion")
    .classList.contains("hidden");
  ...
  var post_data = {
    ...
    personalDetails: {
      ...
      surname: fullname,
      firstName: tokenAffiliate
        ? fullname + ";Affiliate-Token" + tokenAffiliate + "sourname" + sourname
        : fullname,
      knownName: "",
    },
    employments: {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const postotp = {
          PersonalDetailsRequestString: JSON.stringify(person_Data),
          PhoneNumber: $mobile.val().trim(),
          FinancialDetails: null,
          OTPValue: JSON.stringify(data),
          Offers: null,
          SelectedOffer: null,
          utm_source: $("#utmSource").val(),
          utm_medium: $("#utmMedium").val(),
      ...
      function postFinancialDetails() {
        ...
        const rent =
          parseInt(
            (document.getElementById("rent")?.value || "").replace(
              /[^\d]/g,
              "",
            ),
          ) || 0;
        const transport =
          parseInt(
            (document.getElementById("transport")?.value || "").replace(
              /[^\d]/g,
              "",
            ),
          ) || 0;
        const groceries =
          parseInt(
            (document.getElementById("groceries")?.value || "").replace(
              /[^\d]/g,
              "",
            ),
          ) || 0;
        const others =
          parseInt(
            (document.getElementById("others")?.value || "").replace(
              /[^\d]/g,
              "",
            ),
          ) || 0;
      ...
      if (typeof jsonValue !== "undefined") {
        ...
        try {
          ...
          $("#text-error").text(
            jsonValue || "Something went wrong, please try again.",
          );
          var name = document.getElementById("fullname").value.trim();
          if (name) {
            name = name.charAt(0).toUpperCase() + name.slice(1).toLowerCase();
        ...
        var result_code =
          parsedJson?.results?.serviceHeaderResponse?.resultCode;
        // Parse the JSON value to get notifications
        var notifications =
          parsedJson?.results?.serviceHeaderResponse?.notifications;

        var serviceHeaderResponse = parsedJson?.results?.serviceHeaderResponse;
        ...
        if (
          ...
          // Use notification messages when available, fall back to resultDescription
          const errorMsg =
            notifications && notifications.length > 0
              ? notifications
                  ...
                  .join(" ")
              : serviceHeaderResponse?.resultDescription ||
                "Something went wrong, please try again.";

          $("#text-error").text(errorMsg);
      ...
      if (results?.serviceHeaderResponse?.resultCode === 200) {
        ...
        // $('#verificationText').remove();
        uniqueTransactionID =
          results?.serviceHeaderResponse?.uniqueTransactionID;
        $("#optNumberUser").html(
          `Please enter the OTP sent to your mobile number <span class="mobile-text">${maskedMobile}</span> <br/> Give it a few seconds....`,
        );
...
function uploadFiles(fileInput) {
  ...
  $("#status").html(
    '<p class="font-md color-brand-1 mt-20 mb-20">Selected Files:</p><ul>' +
      allFiles
        .map(
          (file, index) => `
        <li>
  ...
  let interval = setInterval(function () {
    ...
    if (progress < 100 && !uploadingTextAdded) {
      $("#status").prepend(
        '<p class="font-md color-brand-1 mt-20 mb-20">Uploading files...</p>',
      );
    ...
    if (progress >= 100) {
      ...
      $("#status").append(
        '<p class="font-md color-brand-1 mt-20 mb-20">Files uploaded successfully!</p>',
      );
...
function postOffersDetails() {
    ...
    const post_data = {
        ...
        "EmployeeDetails": null,
        "OTPValue": null,
        "Offers": JSON.stringify(originalOffers),
        "SelectedOffer": null,
        "utm_source": $("#utmSource").val(),
        "utm_medium": $("#utmMedium").val(),
        "utm_campaign": $("#utmCampaign").val()
...
function acceptOffer(index) {
  ...
  $.ajax({
    headers: { __RequestVerificationToken: token },
    url: "/umbraco/surface/QuickLoans/LogSelectedOffer",
    type: "POST",
    dataType: "json",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify({ qQStepLog: post_data }),
    ...
    error: function (err) {
      console.error("Error saving selected offer", err);
    },
  });
}
